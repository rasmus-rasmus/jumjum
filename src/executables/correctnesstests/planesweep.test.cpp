...
TEST_CASE("planesweep::perform - upper end point, interior point, and lower end point") 
{
  std::vector<primitives::LineSegment> lines = {
      primitives::LineSegment(primitives::Point(0., .1),
                              primitives::Point(1., -.1)),
      primitives::LineSegment(primitives::Point(0, .5),
                              primitives::Point(0, -.5)),
      primitives::LineSegment(primitives::Point(.5, .5),
                              primitives::Point(.5, -.5)),
      primitives::LineSegment(primitives::Point(1., .5),
                              primitives::Point(1., -.5))};
  ...
  CHECK(std::get<primitives::Point>(
            intersections[0].first.computeIntersection(intersections[0].second))
            .squareDistance(primitives::Point(0., 0.1)) < 1e-6);
  CHECK(std::get<primitives::Point>(
            intersections[1].first.computeIntersection(intersections[1].second))
            .squareDistance(primitives::Point(0.5, 0)) < 1e-6);
  CHECK(std::get<primitives::Point>(
            intersections[2].first.computeIntersection(intersections[2].second))
            .squareDistance(primitives::Point(1., -0.1)) < 1e-6);
}
...
TEST_CASE("planesweep::perform - general case") 
{
  ...
  while (std::getline(file, dataLine)) 
  {
    ...
    lines.push_back(primitives::LineSegment(
        primitives::Point(pointCoords[0], pointCoords[1]),
        primitives::Point(pointCoords[2], pointCoords[3])));
  }
